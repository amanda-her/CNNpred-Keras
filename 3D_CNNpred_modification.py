...
def CNN(train_data, test_data, train_target, test_target):
    ...
    for i in range(1,2):
        ...
        if my_file.is_file():
            ...
            # n_feature_maps = 32
            # nb_classes=2
            # input_layer = keras.layers.Input((seq_len, number_feature)  )
            # print("Input  shape 1" + str((number_of_stocks,seq_len, number_feature)))
            # conv_x = keras.layers.Conv1D(filters=n_feature_maps, kernel_size=8, padding='same')(input_layer)
            # print("conv_x "+str(conv_x.shape))
            ...
            # print("conv_z " + str(conv_z.shape))
            # conv_z = keras.layers.BatchNormalization()(conv_z)
            # print("conv_z " + str(conv_z.shape))
            #
            # # expand channels for the sum
            # shortcut_y = keras.layers.Conv1D(filters=n_feature_maps, kernel_size=1, padding='same')(input_layer)
            # print("shortcut_y " + str(shortcut_y.shape))
            # shortcut_y = keras.layers.BatchNormalization()(shortcut_y)
            # print("shortcut_y " + str(shortcut_y.shape))
            #
            # output_block_1 = keras.layers.add([shortcut_y, conv_z])
            # print("output_block_1 " + str(output_block_1.shape))
            ...
            # print("output_block_2 " + str(output_block_2.shape))
            ...
            # print("output_block_3 " + str(output_block_3.shape))
            # output_block_3 = keras.layers.Activation('relu')(output_block_3)
            # print("output_block_3 " + str(output_block_3.shape))
            #
            # # FINAL
            #
            # gap_layer = keras.layers.GlobalAveragePooling1D()(output_block_3) #[0,:,:,:]
            ...
            #
            # model_checkpoint = keras.callbacks.ModelCheckpoint(filepath=filepath, monitor='loss',
            #                                                    save_best_only=True)
            #

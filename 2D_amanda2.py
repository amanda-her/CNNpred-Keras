...
def prepare_target(data):
    ...
    class_array = np.insert(np.array(class_array), 0, 1)
    enc = OneHotEncoder(categories='auto')
    class_array_reshaped = np.array(class_array).reshape(-1, 1)
    enc.fit(class_array_reshaped)
    target = pd.DataFrame(enc.transform(class_array_reshaped).toarray())
    print(target)
...
def costruct_data_warehouse(ROOT_PATH, file_names):
    ...
    for stock_file_name in file_names:
        ...
        target=prepare_target(data['Close'])
...
def run_cnn_ann(data_warehouse):
    ...
    for stock in cnn_results.keys():
        ...
        cnn_results1.to_csv(join(Base_dir, '2D-models/{}/new results-FCN2-200e-128b.csv'.format(stock)), index=False)
